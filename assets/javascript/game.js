...
function game() {
	...
	$("#red-button").attr("value", redValue);
	$("#blue-button").attr("value", blueValue);
	$("#green-button").attr("value", greenValue);
	$("#purple-button").attr("value", purpleValue);
}
...
function checkwin() {
	...
	if (playerPoints > goalPoints) {
		game();
		$("#loss-total").html(losses++).effect("bounce","slow");
	}
	if (playerPoints == goalPoints) {
		game();
		$("#win-total").html(wins++).effect("bounce","slow");
